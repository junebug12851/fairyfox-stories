...
**Version:** `0.4.1` (single source of truth: repo-root `VERSION`).
...
**Seven books — one `complete`, six `growing`.** Latest run **07-16 (v0.4.1, grow)**: three chapters —
*Every Lock but Hers* ch.4 "The Bolt" (4/11), *The Cinderwick Job* ch.5 "The Auction" (5/7), *The
Wintering House* ch.6 "The Woman in the Ground" (6/11). **The random override fired for the first time
in three runs** (slot-1 roll `18`, on the ≤18 threshold) → pure-random draw landed on *The Wintering
House*; the two blend leaders were *Every Lock but Hers* (0.864) and *The Cinderwick Job* (0.714). Honest
consequence: by pure blend the third slot was *The Blindfold Act* (0.667), so the override cost *Blindfold*
its grow this run — it stays 1/6 and leads the next blend. Two books remain **one chapter from done** —
*Girl* (4/5) and *Cartographer* (5/6), both untouched since 07-15 and rising. Full audit in
`sessions/2026-07/2026-07-16.md`. *The Blindfold Act* was planted 07-15 (v0.4.0): a 1926 carnival murder
mystery, the shelf's **first mystery**, **first grounded non-magical realm** (The Sawdust Circuit), and
**first `sequelLock: true` book**; ships with ch.1 + full blueprint, art budget-deferred.

## Next

- **The Blindfold Act leads the next blend.** At 1/6 it is the least-complete book on the shelf, and it
  went untouched 07-16 only because the override fired and took its blend slot. Grown 07-15, so by 07-17
  it is both stale and least-complete — expect it first in line next run.
- **Two books are still one chapter from completion:** *The Girl Who Sold the Wind* (4/5 — next is ch.5
  "The Homesick Wind," the finale) and *The Cartographer of Decks* (5/6 — next is ch.6 "The Chart She
  Draws," the finale). Untouched since 07-15; both rise on the blend as they go stale (near-complete
  cuts the incompleteness term, so they need a stale day or an override to get picked). Expect one to
  **finish** within a run or two — the shelf's second/third completed books, feeding the eligible-sequel
  pool.
...
- **Deferred eyeball (07-11 → 07-16):** Chrome wasn't connected on any recent run (six grow runs + the
  ...
  debt is now **eighteen grown chapter pages + the *Blindfold Act* card and ch.1** deep, worth one
